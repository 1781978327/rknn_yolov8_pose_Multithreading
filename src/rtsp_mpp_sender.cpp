...
static int init_encoder(MppRtspContext* ctx, Command& obj) {
    ...
    ctx->codecCtx->time_base = AVRational{ 1, obj.get_fps() };
    ctx->codecCtx->framerate = AVRational{ obj.get_fps(), 1 };
    ctx->codecCtx->gop_size = 12;
    ...
    if (strstr(ctx->codec->name, "rkmpp")) {
        ...
        if (ctx->hwframe) {
            ...
            if (res >= 0) { ctx->codecCtx->hw_frames_ctx = ctx->hwframe; ctx->codecCtx->hw_device_ctx = ctx->hwdevice; }
        }
    }
    ...
    ctx->stream->time_base = AVRational{ 1, obj.get_fps() };
...
static int send_packet(MppRtspContext* ctx, Command& obj) {
    (void)obj;
    // 基于 mpp 产生的 packet 构造一个新的 AVPacket，
    // 在前面拼接 SPS/PPS（cExtradata），避免直接写 mpp 内部缓冲区。
    ctx->packet->pts = av_rescale_q_rnd(ctx->framecount, ctx->codecCtx->time_base,
                                        ctx->stream->time_base,
                                        (AVRounding)(AV_ROUND_NEAR_INF | AV_ROUND_NEAR_INF));
    ctx->packet->dts = av_rescale_q_rnd(ctx->framecount, ctx->codecCtx->time_base,
                                        ctx->stream->time_base,
                                        (AVRounding)(AV_ROUND_NEAR_INF | AV_ROUND_NEAR_INF));
    ctx->packet->duration = av_rescale_q_rnd(ctx->packet->duration,
    ...
    out.data = (uint8_t*)av_malloc(total_size);
    if (!out.data) {
        print_error(__LINE__, AVERROR(ENOMEM), "alloc out packet");
...
static MPP_RET convert_cvframe_to_drm(MppRtspContext* ctx, cv::Mat& cvframe, Command& obj) {
    ...
    if (ctx->mppPacket) {
        ...
        ctx->packet->pts = mpp_packet_get_pts(ctx->mppPacket);
        ctx->packet->dts = mpp_packet_get_dts(ctx->mppPacket);
        if (ctx->packet->pts <= 0) ctx->packet->pts = ctx->packet->dts;
        if (ctx->packet->dts <= 0) ctx->packet->dts = ctx->packet->pts;
        ctx->meta = mpp_packet_get_meta(ctx->mppPacket);
        int keyframe = 0;
...
static int transfer_frame(MppRtspContext* ctx, cv::Mat& cvframe, Command& obj) {
    convert_cvframe_to_drm(ctx, cvframe, obj);
...
static void context_destroy(MppRtspContext* ctx) {
    if (!ctx) return;
    if (ctx->mpp_initialized) { if (ctx->group) { mpp_buffer_group_put(ctx->group); ctx->group = nullptr; } ctx->mpp_initialized = 0; }
    if (ctx->formatCtx) {
    ...
    if (ctx->codecCtx) { if (ctx->codecCtx->hw_device_ctx) av_buffer_unref(&ctx->codecCtx->hw_device_ctx); avcodec_free_context(&ctx->codecCtx); ctx->codecCtx = nullptr; }
...
bool RtspMppSender::push(cv::Mat& bgr_frame) {
    ...
    if (bgr_frame.cols != (int)ctx->width || bgr_frame.rows != (int)ctx->height) return false;
